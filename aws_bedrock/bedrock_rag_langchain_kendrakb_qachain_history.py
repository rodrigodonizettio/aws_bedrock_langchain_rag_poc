...
# <deprecated> from langchain_community.retrievers.kendra import AmazonKendraRetriever
...
def config_kb_retriever():
    kb_retriever = AmazonKendraRetriever(
        index_id="myKendraIndexID", # AWS Kendra > Indexes > myIndex > Index settings > Index ID
        region_name=AWS_REGION,
        credentials_profile_name=AWS_CREDENTIALS_PROFILE_NAME, # Use valid credentials profile name from ~/.aws/credentials
        top_k=5, # Number of top documents to return
...
while True:
    try:
        ...
        if query.lower()  == 'exit':
            ...
            print('Response Answer is:', response['answer'])
            source_docs = response['source_documents']
            print('\nSource documentation are:')
            for doc in source_docs:
                meta = doc.metadata.get('document_attributes')
                print(f"- {meta.get('_source_uri', 'No URI')} (Page Number: {meta.get('_excerpt_page_number', 'No Page Number')})")      
